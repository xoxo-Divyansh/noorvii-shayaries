      ...
      <div className="relative z-10 flex w-full max-w-2xl flex-col items-center justify-center space-y-6 px-6 py-10 md:max-w-3xl md:px-10 md:py-12">
        ...
        {title && (
          <div className="inline-flex items-center gap-2.5 rounded-full border-2 border-[#6B1F1F] bg-[#fef9f5] px-6 py-2.5 shadow-lg">
            ...
            <span className="font-[var(--font-playfair)] text-sm font-semibold italic tracking-[0.12em] text-[#6B1F1F]">
              {title}
            </span>
          </div>
        )}

        {/* Poetry text - dark maroon, highly visible */}
        <div className="w-full text-center">
          <p 
            className="font-[var(--font-playfair)] text-[1.85rem] leading-[1.8] text-[#3a0f0f] md:text-[2.5rem] md:leading-[1.9]"
            style={{ 
              fontStyle: 'italic',
              fontWeight: 600,
              letterSpacing: '0.025em',
              textShadow: '0 2px 6px rgba(58, 15, 15, 0.25)'
            }}
          >
            {text}
          </p>
        ...
        <div className="text-center">
          <span className="font-[var(--font-playfair)] text-sm font-medium italic tracking-wider text-[#6B1F1F]">
            {mood}
          </span>
        </div>

        {/* Download button - dark maroon */}
        {downloadHref && (
          <a
            href={downloadHref}
            download
            className="group mt-3 inline-flex items-center gap-2.5 rounded-full border-2 border-[#6B1F1F] bg-gradient-to-r from-[#4A1515] to-[#6B1F1F] px-7 py-3 text-sm font-semibold tracking-wide text-white shadow-xl transition-all hover:scale-105 hover:shadow-2xl"
          >
